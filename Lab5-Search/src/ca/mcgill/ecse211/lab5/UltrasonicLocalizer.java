...
public class UltrasonicLocalizer {
   ...
   */
  public UltrasonicLocalizer(Poller poller, Navigation nav) throws OdometerExceptions {
  ...
  public void fallingEdge() {
    ...
    for (int i = 0; i < 2; i++) {
      while (true) {
        if (poller.lastDistance > (D + THRESHOLD) && poller.distance < (D + THRESHOLD)) {
          temp1 = odo.getXYT()[2];
          while (true) {
            if (poller.distance < D - THRESHOLD) {
              temp2 = odo.getXYT()[2];
