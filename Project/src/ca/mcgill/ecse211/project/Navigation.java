...
public class Navigation {
	...
	public void travelToYellowZone(int tunnelLL_x, int tunnelLL_y, int tunnelUR_x, int tunnelUR_y) {
		...
		tunnelEntranceExit(tunnelLL_x, tunnelLL_y,  tunnelUR_x, tunnelUR_y, tunnelEntranceXY, tunnelExitXY);
	...
	/**
	 *  This method finds the coordinates of the entrance and the exit of a 
	 * tunnel relative to the robot's position and stores them in a arrays given as parameters.
	 * 
	 ...
	 */
	public void tunnelEntranceExit(int tunnelLL_x, int tunnelLL_y, int tunnelUR_x, int tunnelUR_y, double[] tunnelEntranceXY, double[] tunnelExitXY) {
		
		int tunnelOrientation; //orientation of the tunnel, ranges from 0 to 3, 0 is entrance left exit right, 1 is entrance down exit up, 2 is entrance right exit left and 3 is entrance up exit down 
		
		
		//define tunnel orientation
		if(tunnelUR_x - tunnelLL_x == 2) //tunnel is horizontal
		{
			if( (double)(tunnelLL_x + 1) * TILE_SIZE > odo.getXYT()[0]) // if middle of the tunnel is at the right of the robot
				tunnelOrientation = 0; //entrance left , exit right
			else tunnelOrientation = 2;// entrance right, exit left
		}
		else //tunnel is vertical
		{
			if( (double)(tunnelLL_y + 1) * TILE_SIZE > odo.getXYT()[1]) //if middle of the tunnel is higher than the robot
				tunnelOrientation = 1; //entrance down , exit up
			else tunnelOrientation = 3;// entrance up, exit down		
		}
	...
	public int travelToRingSet(int rs_x, int rs_y) {
		...
		switch(cornerNumber) {
			case 0:	corner_x = (rs_x - 1) * TILE_SIZE;
					corner_y = (rs_y - 1) * TILE_SIZE;
					break;
			case 1:	corner_x = (rs_x + 1) * TILE_SIZE;
					corner_y = (rs_y - 1) * TILE_SIZE;
					break;
			case 2:	corner_x = (rs_x + 1) * TILE_SIZE;
					corner_y = (rs_y + 1) * TILE_SIZE;
					break;
			case 3:	corner_x = (rs_x - 1) * TILE_SIZE;
					corner_y = (rs_y + 1) * TILE_SIZE;
					break;
	...
	public int nearestCorner(double rs_x, double rs_y) {
		...
		double[][] neighborCorner = {{rs_x - 1 , rs_y - 1},{rs_x - 1 , rs_y + 1}, {rs_x + 1 , rs_y +1 }, {rs_x - 1 , rs_y - 1}} ;
	...
	/**
	 * This method returns the index of the smallest element of 
